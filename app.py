...
if st.button("🔥 KHỞI ĐỘNG CỖ MÁY (1-CLICK AUTO)", type="primary"):
    ...
    with st.status("🚀 Cỗ máy Autonomous đang vận hành...", expanded=True) as status:
        ...
        # --- BƯỚC 1: LẬP OUTLINE VÀ TỔNG QUAN ---
        ...
        res_ch1 = crew_ch1.kickoff() # Đã fix lỗi gọi đúp
        ch1_result = getattr(res_ch1, 'raw', str(res_ch1))
        full_document += "CHƯƠNG 1: TỔNG QUAN DỰ ÁN\n" + ch1_result + "\n\n"
        
        with st.expander("👁️ Xem log Chương 1 (Tổng quan)"): st.write(ch1_result)

        # --- BƯỚC 2: AUTO-EXTRACT USE CASES ---
        time.sleep(2) # Nghỉ nhịp
        st.write("### ⚙️ BƯỚC 2: AI đang tự động nhận diện các Use Case...")
        t_extract_uc = Task(
            description=f"Dựa vào ý tưởng: '{user_idea}'. Hãy liệt kê TÊN của 3 đến 5 Use Case cốt lõi nhất. CHỈ TRẢ VỀ DANH SÁCH gạch đầu dòng, không giải thích. VD:\n- Đăng nhập\n- Thanh toán",
            expected_output="Danh sách Use Case", agent=agent_ba
        )
        crew_extract = Crew(agents=[agent_ba], tasks=[t_extract_uc], verbose=False)
        
        res_ext = crew_extract.kickoff()
        uc_raw_list = getattr(res_ext, 'raw', str(res_ext))
        
        use_cases = [line.replace('-', '').strip() for line in uc_raw_list.split('\n') if '-' in line and len(line) > 5]
        if not use_cases: use_cases = ["Quản lý nghiệp vụ chính", "Xử lý ngoại lệ hệ thống"]
        
        st.info(f"🎯 Đã nhận diện {len(use_cases)} Use Case: {', '.join(use_cases)}")
        ...
        for idx, uc_name in enumerate(use_cases):
            ...
            t_uc_draft = Task(description=f"Viết đặc tả chi tiết cho Use Case: '{uc_name}' (hệ thống: {user_idea}). Bắt buộc: Tên UC, Tiền điều kiện, Luồng chính, Luồng ngoại lệ.", expected_output="Nháp UC", agent=agent_ba)
            t_uc_critic = Task(description=f"Đọc bản nháp UC '{uc_name}'. Tìm ra các lỗ hổng logic, người dùng nhập sai, rớt mạng.", expected_output="Lỗi của UC", agent=agent_qa)
            t_uc_final = Task(description=f"Tổng hợp nháp BA và lời chê của QA. Chốt lại thành bản đặc tả UC '{uc_name}' hoàn hảo nhất. Ghi rõ Business Rules.", expected_output="Chốt UC", agent=agent_tpm)
            
            crew_uc = Crew(agents=[agent_ba, agent_qa, agent_tpm], tasks=[t_uc_draft, t_uc_critic, t_uc_final], verbose=False)
            
            # CƠ CHẾ TRY-CATCH XỬ LÝ RATE LIMIT
            try:
                time.sleep(3) # Nghỉ 3 giây trước mỗi UC
                res_uc = crew_uc.kickoff()
                uc_result = getattr(res_uc, 'raw', str(res_uc))
            except Exception as e:
                if "RateLimit" in str(e) or "429" in str(e):
                    st.warning("⏳ API đang quá tải (Rate Limit). Tạm nghỉ 12 giây để hồi Token...")
                    time.sleep(12) # Nghỉ dài để Groq reset limit
                    res_uc = crew_uc.kickoff() # Thử lại lần 2
                    uc_result = getattr(res_uc, 'raw', str(res_uc))
                else:
        ...
        time.sleep(3)
        st.write("### ⚙️ BƯỚC 4: Viết yêu cầu phi chức năng...")
        t_ch3_draft = Task(description=f"Viết CHƯƠNG 3: YÊU CẦU PHI CHỨC NĂNG cho: {user_idea}. Data, Hiệu năng, Bảo mật.", expected_output="Nháp", agent=agent_ba)
        ...
        res_ch3 = crew_ch3.kickoff()
        ch3_result = getattr(res_ch3, 'raw', str(res_ch3))
        full_document += "\nCHƯƠNG 3: YÊU CẦU PHI CHỨC NĂNG & DỮ LIỆU\n" + ch3_result
        
        st.session_state.final_srs = full_document

        # --- BƯỚC 5: AUTO TEST CASE ---
        if auto_testcase:
            time.sleep(4)
            st.write("### ⚙️ BƯỚC 5: Tự động đẻ Test Case từ SRS...")
            t_testcase = Task(
                description=f"Đọc bản SRS sau:\n{full_document[:3000]}...\nTạo BẢNG TEST CASE (ID, Tên, Steps, Input, Expected) cho các Use Case. Bao phủ cả Positive và Negative.",
                expected_output="Danh sách Test Case", agent=agent_qa
            )
            crew_tc = Crew(agents=[agent_qa], tasks=[t_testcase], verbose=False)
            try:
                res_tc = crew_tc.kickoff()
                st.session_state.final_testcases = getattr(res_tc, 'raw', str(res_tc))
            except Exception as e:
                st.warning("Không thể tạo Test Case do đạt giới hạn Rate Limit ở bước cuối.")
...
if st.session_state.final_srs:
    ...
    with tab2: 
        ...
        else: st.info("Không có Test Case.")
    ...
    st.download_button("📥 TẢI XUỐNG FILE WORD TỔNG HỢP", create_export_docx(), "Auto_SRS_Master.docx", type="primary", use_container_width=True)